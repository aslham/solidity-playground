...
import { createHttpTransport, sendEnvelopeToSentryBackend, } from "./transport.js";
...
if (process.argv.length !== 7) {
    console.error("Invalid number of arguments. Expected [<script>, <json-serialized-envelope>, <config-path>, <dsn>, <release>]");
    process.exit(1);
}
...
const filteredEnvelope = anonymizer.filterOutEventsWithExceptionsNotRaisedByHardhat(envelope);
if (filteredEnvelope[1].length === 0) {
    log("The events weren't raised by Hardhat, so we don't report them");
}
else {
    const anonymizeResult = await anonymizer.anonymizeEventsFromEnvelope(filteredEnvelope);
